...
void HttpRequestInput::add_fea(QString key, QString value) {
    fea[key] = value;
}
...
QNetworkReply* HttpRequestWorker::execute(HttpRequestInput *input) {
    ...
    QUrl temp (input->url_str);

    QNetworkRequest request = QNetworkRequest(temp);

    foreach (QString key, input->fea.keys()) {
        request.setRawHeader (
                            key.toStdString().c_str(),
                            input->fea.value(key).toStdString().c_str()
                    );
    }
...
{
    reply->deleteLater();
    emit on_execution_finished(reply);
}
